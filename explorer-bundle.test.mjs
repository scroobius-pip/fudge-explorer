...
test("materializes the live projections into the Explorer bundle contract", async () => {
  ...
  rows.releases = [
    [7, "release", "1", 1_700_000_000_000, "https://font.example"],
  ];
  ...
  rows.rasterPalette = [
    [42, 0, 238, 133, 62, 1_000_000, 11_700, 234, "raster_profile"],
  ];
  rows.backgrounds = [[42, 255, 255, 255, 1_000_000, 1, "computed", 0]];
  rows.fontObservations = [
    [42, 0, "Example Sans", "Example Sans, sans-serif", 400, 700, 900_000, 3],
  ];
  rows.typeRoles = [
    [42, "body", "Example Sans", 400, 16_000, 24_000, 900_000, 1],
  ];
  ...
  rows.motionAssets = [
    [42, "captures/42.webm", "video/webm", 123, 1_000, 100, 50],
  ];
  rows.videoObservations = [
    [42, 0, "video", 0, 0, 10_000, 5_000, 500_000, 1, "dom_geometry"],
  ];
  rows.gradients = [
    [42, 1, null, 0, "linear", 90_000, 0, 0, 10_000, 5_000, 1, "computed"],
  ];
  rows.legacyColors = [[42, [0xffffff, 0x111111, 0xffffff]]];
  rows.fontSources = [
    [42, 0, 0, "Example Sans", "woff2", "https://font.example/example.woff2"],
  ];
  rows.embeddingRuntime = [
    [
      "current",
      null,
      1_700_000_000_000,
  ...
  assert.deepEqual(bundle.data.color_roles[0], [
    42,
    "text_primary",
    17,
    17,
  ...
  assert.deepEqual(bundle.data.raster_palette[0], [
    42,
    0,
    238,
    133,
  ...
  assert.deepEqual(bundle.data.assignments[0], [
    42,
    "page.home",
    0.95,
    "capture",
  ...
  assert.equal(
    bundle.data.motion_assets[0][1],
    "https://pin.fontofweb.com/42.webm",
  );
...
test("accepts projections observed at different corpus generations", async () => {
  let calls = 0;
  const bundle = await buildExplorerBundle(async (input) => {
      calls += 1;
      ...
      return {
        ...response(
          Object.values(EXPLORER_QUERIES).find(
            (spec) => spec.script === input.script,
          ).headers,
          [],
        ),
  ...
  assert.equal(bundle.data.observed_generation, 77);
});
...
test("continues keyset pagination when a page exactly fills the script limit", async () => {
  ...
  const bundle = await buildExplorerBundle(async (input) => {
    ...
    if (name === "captures") {
      captureCalls += 1;
      assert.equal(
        input.parameters.after_capture_id,
        captureCalls === 1 ? -1 : 2_000,
      );
      return response(
        EXPLORER_QUERIES.captures.headers,
        captureCalls === 1 ? captureRows : [],
      );
    }
...
test("splits bootstrap and deferred projections without changing the full bundle", async () => {
  ...
  const bootstrap = await buildExplorerBootstrap(async (input) => {
    const name = input.script.includes("count(capture_id)")
      ? "captureCount"
      : queryName(input.script) || "capturesPage";
  ...
  assert.deepEqual(
    bootstrapCalls
      ...
      .map((call) => call.name)
      .sort(),
    [...EXPLORER_BOOTSTRAP_QUERY_NAMES].sort(),
  );
  assert.ok(
    bootstrapCalls.every((call) => call.expectedGeneration === undefined),
  );
  assert.deepEqual(bootstrap.data.text_styles, []);
  assert.deepEqual(bootstrap.data.backgrounds, []);
  assert.deepEqual(bootstrap.data.raster_palette, []);
  ...
  const details = await buildExplorerDetails(async (input) => {
    ...
    const rows =
      name === "textStyles" ? [[42, "Example Sans", 400, 16_000, 3, 0]] : [];
    return response(EXPLORER_QUERIES[name].headers, rows);
  }, 77);
  assert.deepEqual(
    detailCalls.map((call) => call.name).sort(),
    [...EXPLORER_DEFERRED_QUERY_NAMES].sort(),
  );
  assert.ok(detailCalls.every((call) => call.expectedGeneration === undefined));
  assert.equal(details.data.observed_generation, 77);
  assert.equal(details.data.text_styles.length, 1);
});

test("loads unfenced capture pages concurrently for bootstrap", async () => {
  const calls = [];
  const progress = [];
  const bootstrap = await buildExplorerBootstrap(
    async (input) => {
  ...
  assert.deepEqual(
    pageCalls.map((call) => call.input.parameters.offset),
    [0, 2_000],
  );
  assert.ok(
    pageCalls.every((call) => call.input.expectedGeneration === undefined),
  );
...
function queryName(script) {
  return Object.entries(EXPLORER_QUERIES).find(
    ([, spec]) => spec.script === script,
  )?.[0];
}
